...
async function connect(id, req) {
    const startPage = hostname + '/servers/';
    let browser, info = {}, time = new Date(); // Capture time at the start
    ...
    try {
        ...
        await page.waitForSelector('input.username', { timeout: to.default });

        await page.type('input.username', process.env.ATERNOS_USER);
        ...
        info.elapsed = new Date() - time; // Calculate elapsed time here in the 'finally' block
        return info;
    }
}
